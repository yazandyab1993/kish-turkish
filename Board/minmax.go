...
func (bs *BoardState) MinMax(depth int32, alpha, beta float32, table *TransposTable) float32 {
	...
	if alreadyChecked, prevValue := table.Request(bs, remainingDepth); alreadyChecked {
	...
	if depth == MaxDepth {
		value := bs.RawBoardValue()
		if depth <= MaximumHashDepth {
			table.Set(bs, value, remainingDepth)
		}
		return value
	}
	...
	options = orderOptions(options, bs.Turn)
	...
	if depth <= MaximumHashDepth {
		table.Set(bs, bestValue, remainingDepth)
	}
...
func orderOptions(options []BoardState, turn TileTeam) []BoardState {
	...
	for i, option := range options {
		scored[i] = scoredOption{board: option, score: option.RawBoardValue()}
	}
	...
	ordered := make([]BoardState, len(options))
	for i := range scored {
		ordered[i] = scored[i].board
	}
