...
func Search(b board.BoardState, output chan PossibleMove) int {
	...
	table := board.NewTable()
	for _, branch := range options {
		branch.SwapTeam()
		go analyzeBranch(branch, table, output)
	}
...
func ValidUiTakes(bs *board.BoardState, forcedIndex int, lastDir [2]int) map[int][]int {
	...
	for i := 0; i < 64; i++ {
		...
		if piece.Full == board.Empty || piece.Team != bs.Turn {
			continue
		}
		...
		for _, direction := range [4][2]int{{0, 1}, {0, -1}, {-1, 0}, {1, 0}} {
			...
			for b := 1; b <= stepMax; b++ {
				if piece.King == board.Pawn {
					...
					if piece.Team == board.White && (direction[0] == 0 && direction[1] == 1) {
						continue
					} //Down (black only)
					if piece.Team == board.Black && (direction[0] == 0 && direction[1] == -1) {
						continue
					} //Up (white only)
				}
